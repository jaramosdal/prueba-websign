...
function STUCaptureButton() {
  this.Bounds; // in Screen coordinates
  this.Text;
  this.Click;
}
  
function STUCaptureRectangle(x, y, width, height) {
  this.x = x;
  this.y = y;
  this.width = width;
  this.height = height;
...
class SignatureForm {
  ...
  async connect() {	
	...
	this.mSignatureMode = false;	
	  ...
	  this.mBtns[0] = new STUCaptureButton();
	  this.mBtns[1] = new STUCaptureButton();
	  this.mBtns[2] = new STUCaptureButton();

	  if (this.mSignatureMode) {
		  
	    // LCD is 800x480; Button positions and sizes are fixed
		this.mBtns[0].Bounds = new STUCaptureRectangle(  0, 431, 265, 48);
		this.mBtns[1].Bounds = new STUCaptureRectangle(266, 431, 265, 48);
		this.mBtns[2].Bounds = new STUCaptureRectangle(532, 431, 265, 48);			
        ...
        this.mBtns[0].Bounds = new STUCaptureRectangle(0, y, w1, h);
		this.mBtns[1].Bounds = new STUCaptureRectangle(w1, y, w2, h);
		this.mBtns[2].Bounds = new STUCaptureRectangle(w1 + w2, y, w3, h);			
		...
		this.mBtns[0].Bounds = new STUCaptureRectangle(x, 0, w, h1);
		this.mBtns[1].Bounds = new STUCaptureRectangle(x, h1, w, h2);
		this.mBtns[2].Bounds = new STUCaptureRectangle(x, h1 + h2, w, h3);
	  }
  ...
  tabletToScreen(penData) {
    ...
    return new STUCapturePoint(penData.x * this.mScaleX, penData.y * this.mScaleY);
  }
  ...
  onCanvasClick(event) { 
    ...
    for (let i = 0; i < this.mBtns.length; i++) {
      if (this.mBtns[i].Bounds.Contains(new STUCapturePoint(posX, posY))) {
        this.mBtns[i].Click();
        break;
      }
    }
